...
TEST_TIMESTAMP_LABEL = "local-dev:eatf-test-tsa:per-digest"
CAVEAT = (
...
def generate_test_timestamp(
    *,
    ...
    """Create a TEST-ONLY RFC 3161 response bound to this package digest."""
    sign_cli = eatf_root / "cli" / "eatf-sign" / "bin" / "eatf-sign.js"
    timestamp_script = eatf_root / "scripts" / "regenerate-test-timestamps.py"
    for path in [sign_cli, timestamp_script]:
        if not path.exists():
            raise FileNotFoundError(str(path))

    digest_code, digest_stdout, digest_stderr = command_output(
        [
            "node",
            str(sign_cli),
            "--payload",
    ...
    digest = digest_stdout.strip().splitlines()[-1] if digest_stdout.strip() else ""
    if (
        digest_code != 0
        or len(digest) != 64
        ...
        raise RuntimeError(
            "Could not derive the EATF canonical digest for the test timestamp: "
            f"{digest_stderr.strip() or digest_stdout.strip() or 'unknown error'}"
        )

    output_path.unlink(missing_ok=True)
    timestamp_code, timestamp_stdout, timestamp_stderr = command_output(
        [
            sys.executable,
            str(timestamp_script),
            "--digest",
            digest.lower(),
            "--output",
            str(output_path),
    ...
    if timestamp_code != 0 or not output_path.is_file() or output_path.stat().st_size == 0:
        raise RuntimeError(
            "Could not create the per-digest EATF test timestamp: "
            f"{timestamp_stderr.strip() or timestamp_stdout.strip() or 'unknown error'}"
        )
    return output_path
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Generate a per-digest RFC 3161 response with EATF's TEST-ONLY TSA.",
    )
    ...
    generated_timestamp_path: pathlib.Path | None = None
    if timestamp and args.generate_test_timestamp:
        parser.error("--timestamp and --generate-test-timestamp are mutually exclusive")
    if (
        not timestamp
        and args.generate_test_timestamp
        and eatf_root is not None
        and eatf_root.exists()
    ):
        generated_timestamp_path = output_dir / ".eatf-test-timestamp.tsr"
        timestamp = str(
            generate_test_timestamp(
                payload_path=payload_path,
                metadata_path=metadata_path,
                output_path=generated_timestamp_path,
                eatf_root=eatf_root,
            )
        )
        timestamp_label = TEST_TIMESTAMP_LABEL
    ...
    if missing:
        ...
        if generated_timestamp_path is not None:
            generated_timestamp_path.unlink(missing_ok=True)
    ...
    try:
        receipt = sign_and_verify(
            ...
            timestamp_label=timestamp_label,  # type: ignore[arg-type]
            scope=args.scope,
            signing_profile=args.signing_profile,
        )
