...
| 3 | Provider inventory fixture | Hashed provider key added/removed under synthetic non-name keys. | `provider_inventory_changed` with added/removed hash-key arrays. | Planned | Keys are local structural handles, not identifiers for legal prose. |
| 4 | Service inventory fixture | Hashed service key added/removed under synthetic non-name keys. | `service_inventory_changed` with added/removed hash-key arrays. | Planned | Service keys must remain hash/synthetic only. |
| 5 | Provider-service-detail fixture | Same provider key with changed aggregate service details. | `provider_service_detail_changed`. | Planned | Output remains aggregate/hash-keyed. |
| 6 | Fetch failure fixture | Timeout, HTTP error, redirect, and unreachable pointer metadata. | Snapshot manifest records per-endpoint failure telemetry. | Planned | Fetch failures are operational observations, not endpoint or legal-status judgments. |
| 7 | Non-XML fixture | PDF/HTML/non-parseable input. | Normalizer records skip/error telemetry, not lane failure. | Planned | Non-XML handling is corpus-shape telemetry only. |
...
| 10 | Missing-signing-input fixture | Signing inputs absent. | Explicit `skipped_missing_signing_inputs`. | Planned | Skips must be visible and not silently treated as verified. |
