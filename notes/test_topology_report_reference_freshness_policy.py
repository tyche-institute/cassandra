#!/usr/bin/env python3
"""Smoke test for topology-report reference freshness policy validation."""
...
MODULE_PATH = WORKSPACE / "notes" / "validate_topology_report_reference_freshness_policy.py"


def load_module():
    spec = importlib.util.spec_from_file_location("validate_topology_report_reference_freshness_policy", MODULE_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError("cannot load topology freshness policy validator module")
...
def main() -> int:
    ...
    assert result["inputs"]["checker_output"]["status"] == "ok", result
    assert result["inputs"]["topology_report"]["status"] == "ok", result
    assert "does not clear warnings" in result["research_caveat"], result

    with tempfile.TemporaryDirectory() as tmp_dir:
        tmp = pathlib.Path(tmp_dir)
        for rel in [
            "notes/topology-report-reference-freshness-policy.md",
            "notes/topology-report-reference-freshness-validation-output.json",
        ...
        validator_path.write_text(
            ...
            + "\n# accidental future wiring: topology-report-reference-freshness-validation-output.json\n",
            encoding="utf-8",
        )
        wired = module.validate(tmp)
        assert wired["status"] == "needs_review", wired
        assert any("topology freshness checker appears wired" in err for err in wired["errors"]), wired
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        for rel in [
            ...
            copy_rel(tmp, rel)
        policy_path = tmp / "notes" / "topology-report-reference-freshness-policy.md"
