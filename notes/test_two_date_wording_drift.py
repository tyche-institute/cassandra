#!/usr/bin/env python3
"""Smoke test for two-date wording drift validation."""
...
SCRIPT = WORKSPACE / "notes" / "validate_two_date_wording_drift.py"
OUTPUT = WORKSPACE / "notes" / "two-date-wording-drift-validation-output.json"

spec = importlib.util.spec_from_file_location("validate_two_date_wording_drift", SCRIPT)
...
assert len(result["figures"]) == 2, result
assert all(item["metadata_dates"] == ["2026-05-20", "2026-05-21"] for item in result["figures"]), result
assert all(not item["missing_caveats"] for item in result["figures"]), result
