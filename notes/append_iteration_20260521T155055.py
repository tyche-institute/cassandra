#!/usr/bin/env python3
"""Append bounded same-date cadence iteration state for 2026-05-21T15:50:55Z."""
...
TS = "2026-05-21T15:50:55Z"
TMP_VALIDATION = "/tmp/cassandra-artifact-index-current-hash-post-20260521T155055Z.json"
FINAL_VALIDATION = "/tmp/cassandra-artifact-index-current-hash-final-20260521T155055Z.json"
...
def run_validation(output_path: str) -> tuple[int, dict]:
    proc = subprocess.run(
        [".venv/bin/python", "notes/validate_artifact_index_current_hashes.py", "--workspace", ".", "--output", output_path],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=False,
    )
    validation = json.loads(Path(output_path).read_text(encoding="utf-8"))
...
def main() -> int:
    ...
    helper_rel = "notes/append_iteration_20260521T155055.py"
...
"""
    ...
    code, validation = run_validation(TMP_VALIDATION)
    ...
    validation_note = (
        ...
        f"it exited `{code}` with status `{validation.get('status')}`, missing_path_count `{validation.get('missing_path_count')}`, "
    ...
    final_code, final_validation = run_validation(FINAL_VALIDATION)
    final_ts = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    final_note = (
        f"\nFinal validation note ({final_ts}): ran an additional artifact-index current-hash validation via "
        f"`.venv/bin/python notes/validate_artifact_index_current_hashes.py --workspace . --output {FINAL_VALIDATION}`; "
        f"it exited `{final_code}` with status `{final_validation.get('status')}`, missing_path_count `{final_validation.get('missing_path_count')}`, "
        f"stale_path_count `{final_validation.get('stale_path_count')}`, duplicate_path_count `{final_validation.get('duplicate_path_count')}`, "
        f"and ignored_self_output_count `{final_validation.get('ignored_self_output_count')}`. This final check was local workflow-maintenance telemetry only; "
        f"no external source was fetched, no dated snapshot was overwritten, and no legal/status assertion, supervision, public alerting, "
        f"regulated trust-service output, legal review, warning clearance, or publication approval was made.\n"
    )
    append(progress, final_note)
    progress_after_final_hash = sha256(progress)
    append(index, artifact_row(
        "HERMES_PROGRESS.md",
        ...
        progress_after_final_hash,
        "yes: final artifact-index current-hash validation exited `0` with status `ok`, missing_path_count `0`, stale_path_count `0`; duplicate rows preserved as workflow-maintenance context; no external source fetched, no snapshot overwrite, no legal/status assertion, supervision, public alerting, regulated trust-service output, legal review, warning clearance, or publication approval."
    ))

    print(json.dumps({
        "status": "ok" if code == 0 and final_code == 0 and validation.get("status") == "ok" and final_validation.get("status") == "ok" else "error",
        "timestamp": TS,
        ...
        "progress_after_final_hash": progress_after_final_hash,
        "validation_exit_code": code,
        "validation_status": validation.get("status"),
        "final_validation_exit_code": final_code,
    ...
    return 0 if code == 0 and final_code == 0 and validation.get("status") == "ok" and final_validation.get("status") == "ok" else 1
