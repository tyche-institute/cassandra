#!/usr/bin/env python3
"""Insert a release-readiness gate-summary note into the Cassandra paper.

The note keeps the gate summary as operator-review workflow telemetry. It is not
publication approval, legal review, public alerting, relying-party validation,
supervision, listed-entity status evidence, or regulated trust-service output.
"""
...
OUTPUT = WORKSPACE / "notes" / "paper-release-gate-summary-note-check-output.json"
MARKER = "The release-readiness gate summary is a snapshot of the checklist state for operator review, not a release decision."
INSERT_AFTER = (
...
def main() -> int:
    ...
    for phrase in FORBIDDEN_UNCAVEATED:
        ...
        while True:
            ...
            if index == -1:
                break
            local = lowered_section[max(0, index - 160): index + len(phrase) + 160]
    ...
    result = {
        "schema": "urn:tyche:cassandra:paper-release-gate-summary-note-check:0.1",
        ...
        "research_caveat": "Release-readiness gate-summary note check only; this does not clear warnings, approve publication, perform legal review, assert legal effect, validate signatures, supervise trusted lists, provide public alerting, or create regulated trust-service output.",
    }
