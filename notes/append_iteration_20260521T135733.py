#!/usr/bin/env python3
"""Append bounded same-date cadence iteration records for 2026-05-21T13:57:33Z.
...
HELPER = ROOT / "notes/append_iteration_20260521T135733.py"

ENTRY_TS = "2026-05-21T13:57:33Z"
DATE_OUTPUT = "2026-05-21T13:57:33.903250084Z"
...
- Ran `.venv/bin/python notes/report_daily_cadence_status.py --workspace . --output notes/daily-cadence-status-output.json`; it exited `0` with status `{cadence.get('status')}`, error_count `{cadence.get('error_count')}`, warning_count `{cadence.get('warning_count')}`, latest completed local lineage `{latest}`, today `{cadence.get('today_utc')}`, today guard status `{cadence.get('today_guard', {}).get('status')}`, next eligible date `{next_date}`, and next eligible guard status `{cadence.get('next_eligible_guard', {}).get('status')}`.
...
append(INDEX, index_row(
    "notes/append_iteration_20260521T135733.py",
...
validation_out = Path("/tmp/cassandra-artifact-index-current-hash-post-20260521T135733Z.json")
proc = subprocess.run(
...
final_out = Path("/tmp/cassandra-artifact-index-current-hash-final-20260521T135733Z.json")
